...
void (*keyboardListener)(unsigned long scancode);
...
int readOutBuf(unsigned long *data){
	...
	while(1){
        ...
        if ((statusbyte & OBF) == 1) { //checks if there's data on the out buffer
...
int sendCmdToKBC(unsigned long reg, unsigned long cmd) {
	...
	unsigned long stat;
        ...
        //Falta por ciclo c/ nr. de tentativas
...
void setKeyboardListener(void (*listener)(unsigned long scancode)) {
