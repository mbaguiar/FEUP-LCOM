...
int dx = 0, dy = 0;
void (*mouseListener)(int dx, int dy, int rb, int lb);
...
void mouse_handler() {
								...
								if (pcktCounter == 0) {
																...
																packet[2] = data;
																pcktCounter = 0;
																int lb = packet[0] & LB; int rb = ((packet[0] & RB) >> 1); dx = getXY().x; dy = getXY().y;
																mouseListener(dx, dy, rb, lb);
								}
...
void setMouseListener(void (*listener)(int dx, int dy, int rb, int lb)){
